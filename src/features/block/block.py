...
def build_one_instrument_blocks(symbol, frequency):
    ...
    if not last_2_docs_df.empty:
        ...
        del df['_id']
        if length == 2:
            block_df = pd.concat([df.head(1), block_df], ignore_index=True, sort=False)
    ...
    if not last_2_docs_df.empty:
        ...
        if last_doc_s.equals(first_doc_s):
            ...
            record_id = last_2_docs_df['_id'].iloc[-1]
            # record_id = ObjectId(record_id.values[0])
    ...
    result = block_cursor.insert_many(block_df.to_dict('records'))
